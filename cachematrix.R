# These two functions, for a given matrix, cache and returns its inverse.
...
cacheSolve <- function(x, ...) {
   ...
   # If the cache wasn't empty, return its contents (the inverse of 'x')
   if(!is.null(m)) {
   ...
   # If the cache was empty ('m' was NULL), calculate and cache the inverse now.
   data <- x$get()
   m <- solve(data, ...)
   x$setInverse(m)
